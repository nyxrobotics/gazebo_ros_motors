...
void GazeboRosMotor::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
  ...
  // ode_joint_motor_fmax_ = maximum force the ODE joint-motor may apply during
  // a time step
  gazebo_ros_->getParameter<double>(ode_joint_motor_fmax_,
                                    "ode_joint_motor_fmax", 50.0);
  gazebo_ros_->getParameter<double>(ode_joint_fudge_factor_,
  ...
  gazebo_ros_->getParameter<int>(encoder_pulses_per_revolution_, "encoder_ppr",
                                 4096);
  gazebo_ros_->getParameter<double>(encoder_to_shaft_ratio_,
  ...
  ROS_INFO_NAMED("motor_plugin", "Wrench frame: %s",
  ...
  if (this->publish_motor_joint_state_) {
    ...
    ROS_INFO_NAMED("motor_plugin", "%s: Advertise joint_state",
                   gazebo_ros_->info());
  }

  // command subscription
  last_update_time_ = parent->GetWorld()->SimTime();
  ROS_INFO_NAMED("motor_plugin", "%s: Trying to subscribe to %s",
  ...
  ROS_INFO_NAMED("motor_plugin", "%s: Subscribed to %s", gazebo_ros_->info(),
                 command_topic_.c_str());

  if (this->publish_velocity_) {
    velocity_publisher_ =
        gazebo_ros_->node()->advertise<std_msgs::Float32>(velocity_topic_, 1);
    ROS_INFO_NAMED("motor_plugin", "%s: Advertising shaft velocity on %s ",
  ...
  if (this->publish_encoder_) {
    ...
    ROS_INFO_NAMED("motor_plugin", "%s: Advertising encoder counts on %s ",

...
// Destructor
GazeboRosMotor::~GazeboRosMotor() { FiniChild(); }
...
void GazeboRosMotor::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
  ...
  this->callback_queue_thread_ =
      std::thread(std::bind(&GazeboRosMotor::QueueThread, this));

  // global parameters
  gazebo_ros_->getParameter<std::string>(command_topic_, "command_topic",
                                         "/motor/voltage_norm");
  // motor model parameters
  gazebo_ros_->getParameter<double>(motor_nominal_voltage_,
                                    "motor_nominal_voltage",
                                    24.0);  // Datasheet 24.0V
  ROS_INFO_NAMED(plugin_name_, "motor_nominal_voltage_ = %f",
                 motor_nominal_voltage_);
  gazebo_ros_->getParameter<double>(moment_of_inertia_, "moment_of_inertia",
                                    0.001);  // 0.001 kgm^2
  ROS_INFO_NAMED(plugin_name_, "moment_of_inertia_ = %f", moment_of_inertia_);
  gazebo_ros_->getParameter<double>(
      armature_damping_ratio_, "armature_damping_ratio", 0.0001);  // Nm/(rad/s)
  ROS_INFO_NAMED(plugin_name_, "armature_damping_ratio_ = %f",
                 armature_damping_ratio_);
  gazebo_ros_->getParameter<double>(
      electromotive_force_constant_, "electromotive_force_constant",
      0.08);  // Datasheet: 1.8Nm / 22A or 24V / 300 (rad/s)
  ROS_INFO_NAMED(plugin_name_, "electromotive_force_constant_ = %f",
                 electromotive_force_constant_);
  gazebo_ros_->getParameter<double>(electric_resistance_, "electric_resistance",
                                    1.0);  // 1 Ohm
  ROS_INFO_NAMED(plugin_name_, "electric_resistance_ = %f",
                 electric_resistance_);
  gazebo_ros_->getParameter<double>(electric_inductance_, "electric_inductance",
                                    0.001);  // 1 mH
  ROS_INFO_NAMED(plugin_name_, "electric_inductance_ = %f",
                 electric_inductance_);

  // noise parameters
  gazebo_ros_->getParameter<double>(velocity_noise_, "velocity_noise", 0.0);

  // gearbox parameters
  gazebo_ros_->getParameter<double>(gear_ratio_, "gear_ratio",
                                    1.0);  // Reduction!
  ...
  gazebo_ros_->getParameter<int>(encoder_pulses_per_revolution_, "encoder_ppr",
                                 4096);

  gazebo_ros_->getParameter<std::string>(velocity_topic_, "velocity_topic",
                                         "/motor/velocity");
  gazebo_ros_->getParameter<std::string>(encoder_topic_, "encoder_topic",
                                         "/motor/encoder");
  gazebo_ros_->getParameter<std::string>(current_topic_, "current_topic",
                                         "/motor/current");
  gazebo_ros_->getParameter<std::string>(supply_topic_, "supply_topic",
                                         "/motor/supply_voltage");

  // motor joint
  joint_ = gazebo_ros_->getJoint(parent, "motor_shaft_joint", "shaft_joint");

  // shaft link
  gazebo_ros_->getParameter<std::string>(wrench_frame_, "motor_wrench_frame",
                                         "wheel_link");
  ...
  gazebo_ros_->getParameterBoolean(publish_motor_joint_state_,
                                   "publish_motor_joint_state", false);
  if (this->publish_motor_joint_state_) {
    joint_state_publisher_ =
        gazebo_ros_->node()->advertise<sensor_msgs::JointState>(
            joint_->GetName() + "/joint_state", 1000);
    ROS_INFO_NAMED(plugin_name_, "%s: Advertise joint_state",
                   gazebo_ros_->info());
  }

  last_update_time_ = parent->GetWorld()->SimTime();
  // command subscriber
  ROS_INFO_NAMED(plugin_name_, "%s: Trying to subscribe to %s",
                 gazebo_ros_->info(), command_topic_.c_str());
  ros::SubscribeOptions so = ros::SubscribeOptions::create<std_msgs::Float32>(
      command_topic_, 1, boost::bind(&GazeboRosMotor::cmdVelCallback, this, _1),
      ros::VoidPtr(), &queue_);
  cmd_vel_subscriber_ = gazebo_ros_->node()->subscribe(so);
  ROS_INFO_NAMED(plugin_name_, "%s: Subscribed to %s", gazebo_ros_->info(),
                 command_topic_.c_str());

  // supply voltage subscriber
  ROS_INFO_NAMED(plugin_name_, "%s: Trying to subscribe to %s",
                 gazebo_ros_->info(), supply_topic_.c_str());
  ...
  if (this->publish_velocity_) {
    velocity_publisher_ =
        gazebo_ros_->node()->advertise<std_msgs::Float32>(velocity_topic_, 1);
    ROS_INFO_NAMED(
        plugin_name_,
        "%s: Advertising motor shaft (before gearbox) velocity on %s ",
        gazebo_ros_->info(), velocity_topic_.c_str());
  }
  if (this->publish_encoder_) {
    encoder_publisher_ =
        gazebo_ros_->node()->advertise<std_msgs::Int32>(encoder_topic_, 1);
    ROS_INFO_NAMED(plugin_name_, "%s: Advertising encoder counts on %s ",
                   gazebo_ros_->info(), encoder_topic_.c_str());
  }
  if (this->publish_current_) {
    current_publisher_ =
        gazebo_ros_->node()->advertise<std_msgs::Float32>(current_topic_, 1);
    ROS_INFO_NAMED(plugin_name_, "%s: Advertising actual motor current on %s ",
                   gazebo_ros_->info(), current_topic_.c_str());
  }

  // listen to the update event (broadcast every simulation iteration)
  this->update_connection_ = event::Events::ConnectWorldUpdateBegin(
      boost::bind(&GazeboRosMotor::UpdateChild, this));
  ...
  ROS_INFO_NAMED(plugin_name_, "%s: Setting up dynamic reconfigure server.",
                 gazebo_ros_->info());
  node_handle_ = new ros::NodeHandle(plugin_name_);
  dynamic_reconfigure_server_.reset(
      new dynamic_reconfigure::Server<gazebo_ros_motors::motorModelConfig>(
          reconf_mutex_, ros::NodeHandle(*node_handle_)));
  boost::recursive_mutex::scoped_lock scoped_lock(reconf_mutex_);
  notify_server_ = true;
  this->paramServerUpdate();
  dynamic_reconfigure_server_->setCallback(
      boost::bind(&GazeboRosMotor::reconfigureCallBack, this, _1, _2));
  scoped_lock.unlock();
}
...
bool GazeboRosMotor::checkParameters() {
  if (this->armature_damping_ratio_ != 0.0 &&
      !std::isnan(this->armature_damping_ratio_) &&
      this->electric_inductance_ != 0.0 &&
      !std::isnan(this->electric_inductance_) &&
      this->electric_resistance_ != 0.0 &&
      !std::isnan(this->electric_resistance_) &&
      this->electromotive_force_constant_ != 0.0 &&
      !std::isnan(this->electromotive_force_constant_) &&
      this->moment_of_inertia_ != 0.0 && !std::isnan(this->moment_of_inertia_))
    return true;
  else
    return false;
}
...
bool GazeboRosMotor::ValidateParameters() {
  ...
  bool ok = true;
  // Check if d^2 L^2 + J^2 R^2 - 2 J L (2 Km^2 + d R) > 0 (which appears under
  // sqrt)
  double Om = 0;
  if (d * d * L * L + J * J * R * R > 2 * J * L * (2 * Km * Km + d * R)) {
    Om =
        sqrt(d * d * L * L + J * J * R * R - 2 * J * L * (2 * Km * Km + d * R));
    // OK, roots are real, not complex
  } else {
    ok = false;
    ROS_WARN_NAMED(plugin_name_,
                   "Incorrect DC motor parameters: d^2 L^2 + J^2 R^2 - 2 J L "
                   "(2 Km^2 + d R) > 0 not satisfied!");
  }
  ...
  if (ok) {
    ...
    if (Om < d * L + J * R) {
      ...
      ok = false;
      ROS_WARN_NAMED(plugin_name_,
                     "Incorrect DC motor parameters: sqrt(d^2 L^2 + J^2 R^2 - "
                     "2 J L (2 Km^2 + d R)) < d*L+J*R not satisfied!");
    }
  }
  return ok;
}
...
void GazeboRosMotor::paramServerUpdate() {
  if (notify_server_) {
    ...
    current_config_.electromotive_force_constant =
        electromotive_force_constant_;
...
void GazeboRosMotor::reconfigureCallBack(
    const gazebo_ros_motors::motorModelConfig& config, uint32_t level) {
  if (this->checkParameters()) {
    ...
    double temp =
        this->electric_resistance_;  // Temporary storage for parameter
    ...
    if (electric_resistance_ != config.electric_resistance) {
      ...
      if (!this->ValidateParameters()) {
        ...
        ROS_WARN_NAMED(plugin_name_,
                       "Electric resistance %6.3f discarded, keeping previous "
                       "value of %6.3f Ohm",
    ...
    if (electric_inductance_ != config.electric_inductance) {
      ...
      if (!this->ValidateParameters()) {
        ...
        ROS_WARN_NAMED(plugin_name_,
                       "Electric inductance %6.3f discarded, keeping previous "
                       "value of %6.3f H",
    ...
    if (moment_of_inertia_ != config.moment_of_inertia) {
      ...
      if (!this->ValidateParameters()) {
        ...
        ROS_WARN_NAMED(plugin_name_,
                       "Moment of inertia %6.3f discarded, keeping previous "
                       "value of %6.3f kgm^2",
    ...
    if (armature_damping_ratio_ != config.armature_damping_ratio) {
      ...
      if (!this->ValidateParameters()) {
        ...
        ROS_WARN_NAMED(plugin_name_,
                       "Armature damping %6.3f discarded, keeping previous "
                       "value of %6.3f Nm/(rad/s)",
    ...
    if (electromotive_force_constant_ != config.electromotive_force_constant) {
      electromotive_force_constant_ =
          config.electromotive_force_constant;  // Km
      if (!this->ValidateParameters()) {
        electromotive_force_constant_ = temp;
        ROS_WARN_NAMED(plugin_name_,
                       "Motor constant %6.3f discarded, keeping previous value "
                       "of %6.3f Nm/(rad/s)",
                       config.electromotive_force_constant,
                       electromotive_force_constant_);
        ok = false;
      }
    }
    ...
    if (ok) {
      ...
      notify_server_ = true;  // Some of the params were rejected, notify the
                              // param server from the main loop.
    }
  }
}
...
void GazeboRosMotor::publishEncoderCount(double m_vel, double dT) {
  ...
  encoder_counter_ +=
      round(((rev_in_rad) / 2 * M_PI) * encoder_pulses_per_revolution_);
...
void GazeboRosMotor::motorModelUpdate(double dt, double output_shaft_omega,
...
void GazeboRosMotor::UpdateChild() {
  ...
  motorModelUpdate(seconds_since_last_update, current_output_speed,
                   actual_load);
  publishWheelJointState(current_output_speed, current_torque.Z());
  publishMotorCurrent();
  auto dist = std::bind(
      std::normal_distribution<double>{current_output_speed, velocity_noise_},
...
void GazeboRosMotor::cmdVelCallback(
    const std_msgs::Float32::ConstPtr& cmd_msg) {
  input_ = cmd_msg->data;
}

void GazeboRosMotor::supplyVoltageCallBack(
